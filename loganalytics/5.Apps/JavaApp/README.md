## Java based prometheus-demo  – Kubernetes Manifest Pack

Little fake metric data generator written in [Java](https://www.java.com/en/)
...
Now click on the top block where it says `Upload dashboard JSON file` and navigate to the `JavaApp/dashboard` directory and select `dashboard-java-full.json`.
...
```bash
# Stop
kubectl scale deployment/java-prometheus-demo -n prometheus-demo --replicas=0
# Restart
kubectl scale deployment/java-prometheus-demo -n prometheus-demo --replicas=1
```
